    ...
    : productions(productions), start_symbol(std::move(start)), end_symbol(std::move(end))
{
    ...
    calculate_firstSets();
    construct_tables();
    // std::cout << "==== print_firstSet ====" << std::endl;
    // print_firstSet();
    // std::cout << "==== print_tables ====" << std::endl;
    // print_tables();
}
    ...
    while (changed)
    {
        ...
        for (const auto &item : old_set)
        {
            ...
            if (get_lr1item_state(item) != LR1Item::State::GOTO)
                continue;
            if (vis_item.find(item) != vis_item.end())
                continue;
...
void LR1Parser::print_stacks(const std::stack<size_t> &stateStack,
...
Action LR1Parser::get_next_action(const Symbol &currentSymbol)
{
    ...
    // std::cout << "当前状态: " << currentState << ", 当前符号: " << currentSymbol.to_string() << std::endl;
    if (actionIt != actionTable.end())
    {
        const Action &action = actionIt->second;
        // std::cout << "找到动作: " << action.to_string() << std::endl;
        return action;
    }
    else
    {
        std::cerr << "get_next_action error: no action" << std::endl;
        return {};
    }
}
